...
require_once($CFG->dirroot . '/user/profile/lib.php');
 ...
 // Adds a dedicated "Face-to-Face" category under Activity modules, grouping
 // all Face-to-Face settings together and introducing a "Bulk Upload Sessions"
 // link. The if ($ADMIN->fulltree) condition ensures these settings load only
 // when the full admin tree is built.
if ($ADMIN->fulltree) {

    // Locate the existing Face-to-Face node.
    $facetofacepage = $ADMIN->locate('modsettingfacetoface');

    // If found, add settings to that page so they appear under Face-to-Face.
    if ($facetofacepage instanceof admin_settingpage || $facetofacepage instanceof admin_category) {
        $settings = $facetofacepage;
        ...
        // Settings from Face to Face module.
        $settings->add(new admin_setting_configtext(
        'facetoface/fromaddress',
        get_string('setting:fromaddress_caption', 'facetoface'),
        get_string('setting:fromaddress', 'facetoface'),
        ...
        $settings->add(new admin_setting_heading('facetoface/cost_header',
        get_string('costheading', 'facetoface'), ''));
        ...
        $settings->add(new admin_setting_heading('facetoface/icalendar_header',
        get_string('icalendarheading', 'facetoface'), ''));
